...
USBD_Status  USBD_StdItfReq (USB_OTG_CORE_HANDLE  *pdev, USB_SETUP_REQ  *req)
{
  ...
  case USB_OTG_CONFIGURED:
    ...
    {
#if 1
        //distribute request to correct interface
      if (LOBYTE(req->wIndex) == USB_HID_IF_NUM)
      {
        pdev->dev.class_cb2->Setup (pdev, req);
      }
      else
      {
        pdev->dev.class_cb1->Setup (pdev, req);
      }
#else
...
#if 1
      if ((ep_addr == HID_IN_EP) || (ep_addr == HID_OUT_EP))
      {
          pdev->dev.class_cb2->Setup (pdev, req);
      }
      else if ((ep_addr == MSC_IN_EP) || (ep_addr == MSC_OUT_EP))
      {
          pdev->dev.class_cb1->Setup (pdev, req);
      }
                               ...
                               USB_SETUP_REQ *req)
{
  ...
  case USB_DESC_TYPE_CONFIGURATION:
#if 1
      pbuf   = (uint8_t *)pdev->dev.class_cb1->GetConfigDescriptor(pdev->cfg.speed, &len);
#else
    ...
    {
#if 1
      pbuf   = (uint8_t *)pdev->dev.class_cb1->GetOtherConfigDescriptor(pdev->cfg.speed, &len);
#else
...
* @retval status
*/
void USBD_ClrFeature(USB_OTG_CORE_HANDLE  *pdev,
                            USB_SETUP_REQ *req)
{
  switch (pdev->dev.device_status)
  {
