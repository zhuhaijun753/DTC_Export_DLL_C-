...
#if FORD_use_sSystem
  {
    if (p_strtOemProfile->Protocol == PROTOCOL_DDL1)
      p_strtKeyInfo->sSystem = p_strtOemProfile->sSystem;
  }
#endif  //FORD_use_sProtocol
#if FORD_use_sProtocol
  {
    if (p_strtOemProfile->Protocol == PROTOCOL_DDL1)
      p_strtKeyInfo->bProtocol = PROTOCOL_DDL1;
  }
#endif
}
...
*     1. Creation Lap Dang Sep 09, 2014
-------------------------------------------------------------------------------*/
uint32 gf_FORD_Adapter_DTC_GetDef(structMFRVinProfile * p_strtMFRVinProfile,
