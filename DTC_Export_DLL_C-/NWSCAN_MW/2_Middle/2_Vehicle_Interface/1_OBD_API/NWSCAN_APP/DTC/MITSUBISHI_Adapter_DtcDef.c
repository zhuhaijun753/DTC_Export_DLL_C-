...
{	
  ...
  structOemInfo_Mitsubishi *pstrtOemInfo_Mitsubishi;
  pstrtOemInfo_Mitsubishi = (structOemInfo_Mitsubishi*) p_strtOemProfile->OemInfo;
#if MITSUBISHI_use_sManufacturer
  {
    p_strtKeyInfo->sManufacturer = p_strtMFRVinProfile->eManufacturer;
  }
#endif
#if MITSUBISHI_use_sSystem
  {
    p_strtKeyInfo->sSystem = p_strtOemProfile->sSystem;
  }
#endif
#if MITSUBISHI_use_sSubSystem
  {
    p_strtKeyInfo->sSubSystem = p_strtOemProfile->sSubSystem;
  }
#endif
#if MITSUBISHI_use_sProtocol
  {
    p_strtKeyInfo->bProtocol = p_strtOemProfile->Protocol;
  }
#endif
  if (pstrtOemInfo_Mitsubishi->bStatus==VALID_CODE)
    p_strtKeyInfo->sOption3 = pstrtOemInfo_Mitsubishi->sSCC; // aThuyet update CCS
}
...
*     1. Creation Lap Dang Sep 09, 2014
-------------------------------------------------------------------------------*/
uint32 gf_MITSUBISHI_Adapter_DTC_GetDef(structMFRVinProfile * p_strtMFRVinProfile,
...

{
  ...
  eListSCC[0] = strtKeyInfo.sOption3;
  eListSCC[1] = 0xFFFF;
  uint8 bIdxGroup = 0;
  while (pListInnGroup[bIdxGroup] != einnovagroup_UNKNOWN)
  {
    ...
    for (uint8 i=0;i<2;i++)
    {
      strtKeyInfo.sOption3 = eListSCC[i];
