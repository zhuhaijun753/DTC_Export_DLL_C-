                             ...
                             structKeyInfo *p_strtKeyInfo)
{
  ...
  structOemInfo_Renault *pstrtOemInfo_Renault;
  pstrtOemInfo_Renault = (structOemInfo_Renault*) p_strtOemProfile->OemInfo;
#if RENAULT_use_sManufacturer
  {
    p_strtKeyInfo->sManufacturer = p_strtMFRVinProfile->eManufacturer;
  }
#endif
#if RENAULT_use_sSystem
  {
    p_strtKeyInfo->sSystem = p_strtOemProfile->sSystem;
  }
#endif
#if RENAULT_use_sSubSystem
  {
    p_strtKeyInfo->sSubSystem = p_strtOemProfile->sSubSystem;
  }
#endif
#if RENAULT_use_sProtocol
  {
    p_strtKeyInfo->bProtocol = p_strtOemProfile->Protocol;
  }
#endif
  if (pstrtOemInfo_Renault->bStatus == VALID_CODE)
    p_strtKeyInfo->sOption3 = pstrtOemInfo_Renault->eDTCTable;
}
...
*     1. Creation Lap Dang Sep 09, 2014
-------------------------------------------------------------------------------*/
uint32 gf_RENAULT_Adapter_DTC_GetDef(structMFRVinProfile * p_strtMFRVinProfile,
                                  structOemProfile *p_strtOemProfile, enumOBDLanguage eMemLanguage,
                                  structListDTCType *pstrtListDTCType, void *pDef, 
                                  uint32 iDefSize,structListAddressDTC* pstrtListAddrDTC)
