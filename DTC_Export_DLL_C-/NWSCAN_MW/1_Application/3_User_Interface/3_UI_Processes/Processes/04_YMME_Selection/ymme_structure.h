...
typedef struct  _structYMMEOBD1Profile
{
	...
	uint8	        eTransmission;
	uint8            eVehSpeed;
	uint8	        eSpeedDenLap Dangy;
	uint8	            eYear84_5;
	uint8                 eMaf;
	uint8	     eFederalEmission;
	uint8      eVarFuelVehicle;
	uint8              eReatta;
	uint8       eVin10_17_val2;
#else    
    enumOBD1year				eYear;			
    enumOBD1vin8				eVin8;			
...
typedef struct  _structMFRVinProfile
{
    ...
    union
    {
        uint8                  bReserved[VINPROFILE_INFO_MAX_SIZE - (YMME_PROFILE_VIN_NUMBER_VALID_CHARS+1) - 18]; //Note: change Size after modifying this structure
